...
fclose(fd);
	...
	if (sno[0] == '*')
	{
		sprintf(sql, "select sno,sname,sage,ssex,stellphone from information where statu=0");
	}
	else
	{
		sprintf(sql, "select sno,sname,sage,ssex,stellphone from information where and sno = %d ", atoi(sno));
	}
